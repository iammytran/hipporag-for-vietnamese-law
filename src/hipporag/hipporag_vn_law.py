...
class HippoRAGVnLaw:
    ...
    def pre_openie(self,  docs: List[str]):
        ...
        if len(chunk_keys_to_process) > 0:
            ...
            # Lưu file NER Results
            with open('outputs/ner_results.json', 'w', encoding='utf-8') as f_ner:
                json.dump(new_ner_results_dict, f_ner, ensure_ascii=False, indent=4)

            # Lưu file Triple Results
            with open('outputs/triple_results.json', 'w', encoding='utf-8') as f_triple:
                json.dump(new_triple_results_dict, f_triple, ensure_ascii=False, indent=4)

            logger.debug("Đã lưu xong 2 file: ner_results.json và triple_results.json")
