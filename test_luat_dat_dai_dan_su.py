...
def main():
    # Lấy 3 văn bản lớn
    content_land_law, content_civil_law, full_text = load_law_docs()
    ...
    print("\n[1] Đang xử lý Luật Đất Đai...")
    marriage_chunks = chunk_text_by_tokens(content_land_law, encoding, CHUNK_SIZE)
    ...
    queries = [
        "Tìm các bản án có liên quan đến chủ đề Tranh chấp Hợp đồng?",
    ]

    # For Evaluation
    answers = [
        ["Bản án Phúc thẩm số 178/2024/DS-PT"],
    ]
...
def load_law_docs():
    ...
    land_url = 'https://thuvienphapluat.vn/van-ban/Quyen-dan-su/Luat-Hon-nhan-va-gia-dinh-2014-238640.aspx'
    ...
    try:
        ...
        print(f"Đang cào dữ liệu từ: {land_url}")
        response_marriage = requests.get(land_url, headers=headers)
        ...
        if parent_div_marriage:
            ...
            content_land_law = parent_div_marriage.get_text(separator='\n', strip=True)
            print("--- Lấy thành công nội dung Luật Đất Đai ---")
        else:
            print(f"Lỗi: Không tìm thấy div nội dung (class='content1') tại {land_url}")
    ...
    file_path = os.path.join(script_dir, 'data', 'ban_an_dan_su_2.pdf')
    ...
    try:
        ...
        #     print(f"Đang process trang {i + 1}")
    ...
    return content_land_law, content_civil_law, full_text
